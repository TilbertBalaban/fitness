...
- **D-33:** **D-29's sync-rules clause is withdrawn. `ops/powersync/sync-rules.yaml` keeps streaming every `routine_day` row, archived or not; the archived-day filter lives in `loadProgramTree` alone.** D-29 as originally recorded asked for both a filtered pull query and an Archive/Restore Day control, which cannot both hold: PowerSync removes a row from the local database when it leaves the user's sync bucket, so an `archived_at` predicate on the pull query would delete the row from every other device the moment the stamp landed — leaving Restore with nothing to restore and `workout_session.routine_day_id` pointing at a row the device no longer holds. The `routine` table is the precedent and does the opposite: it streams archived programs, which is why the library's Restore works today. `loadProgramTree` is the single day-listing read shared by the builder deck, `loadNextUp` and `duplicateRoutine`, so filtering there is what makes all three agree; `loadArchivedDays` is the one deliberate exception, mirroring the `loadRoutines` / `loadLibraryRoutines` split one level down. The deviation is recorded as a comment in `sync-rules.yaml` — a deliberate deviation plus an external-contract quirk, both sanctioned by the house comment rule. — **Reversibility:** reversible; `routine_day.archived_at` itself stays one-way once synced.

  > **Day-level copy and restore surface, confirmed with the user 2026-08-28.** Archive dialog: heading **"Archive Day"**, body **"Archiving removes it from this program, but any workouts you logged from it stay in your history. Archive anyway?"**, confirm **"Archive"**. Restore dialog: heading **"Restore Day"**, body **"This day will reappear in this program."**, confirm **"Restore"**. Restore surface: an **"Archived days"** section below the day deck, rendered only when the routine has archived days (the own-empty-omits-header convention `DetailSection` and the library already follow), each row receded at `opacity 0.6` like an archived library row, with a 48x48 Restore control. The day page's existing **Remove** (hard delete, cascade-tested) stays — replacing it with Archive was considered and rejected as outside this run's blast radius.
