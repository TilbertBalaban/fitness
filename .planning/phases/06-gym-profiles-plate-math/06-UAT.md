...
[testing complete — 30 auto-covered by passing tests; 5 human checkpoints relocated to ROADMAP Phase 999.2]

## Tests

### 1. A user with no configured gym gets exactly one seeded 'My Gym' profile the first time a session starts, and the session row carries its id
...
### 2. The breakdown honours recorded pair counts, proven by a one-pair inventory case
expected: The breakdown honours recorded pair counts, proven by a one-pair inventory case
result: pass
source: automated
coverage_id: 06-01-D3

### 3. A gym profile round-trips to Postgres under the authenticated user, and a malformed inventory is rejected
...
### 4. roundToAchievable has no default rounding direction; a halfway target between two achievable loads resolves to the lower one
...
### 5. A not-loadable barbell target names its two nearest achievable neighbours, with either side null when none exists on that side
...
### 6. One resolveEquipmentBand function answers what the band shows for every equipment type, exhaustively over all twelve EQUIPMENT_TYPES members, and hasResolvableEquipment is the single shared predicate
...
### 7. warmupSets accepts an optional gym-aware roundWeight closure with every existing caller byte-identical
...
### 8. Setting a gym active moves the single active pointer; it never changes any other gym's row, and archiving the active gym leaves the pointer resolvable rather than dangling.
...
### 9. Archived gyms sit in a collapsed trailing section, the active gym is pinned first with accent styling, and the row subtitle joins only configured sections in fixed order — never a zero count.
...
### 10. The Gym Profile Action Sheet's action list is dynamic per row state — Set Active omitted on the active row, Restore in place of Archive on an archived row.
...
### 11. The Profile tab's Gym Profiles row names the active gym when it resolves, is absent otherwise, and always navigates — never disabled or broken.
...
### 12. A user can configure a gym's bar weight (via preset chips or manual entry), plate denominations with per-denomination pair counts, dumbbell weights, and named machines with stack min/max/increment/starting-resistance, and pick the profile's unit system — through one shared create/edit form.
...
### 13. Every weight is stored in canonical kilograms regardless of the profile's unit, and a draft-to-profile-to-draft round trip preserves the exact stored value — no floating-point arithmetic on a parsed decimal.
...
### 14. A plate count stepper cannot go below zero or express a decimal/negative; a duplicate denomination merges into the existing row rather than creating a second one.
...
### 15. A failed local write on Save renders the shipped error surface above the field stack, and the form stays open with every entered value preserved; sync failure is not surfaced inline.
...
### 16. The create route saves through createEquipmentProfile; the edit route loads the profile, seeds the draft, and saves through updateEquipmentProfile. Editing an existing gym's plate count changes only that column.
...
### 17. Setting a gym active moves the preference pointer (proven by Set Active disappearing from that row's own action list on reopen), and archiving moves the row into the archived partition with an archive timestamp, leaving the row present.
...
### 18. The session's resolved inventory is read through loadSessionInventory (D-17 snapshot) — a session started at one gym never re-resolves against a later-switched active gym
...
### 19. PlateStripView renders every EquipmentBandState kind (plates, pair, stack, not_loadable, no_plates, collapsed) per the UI-SPEC Copywriting Contract, grows past the 40px reservation only for a real tap target, and collapses defensively on a thrown computation
...
### 20. The band is live on the workout screen for the currently focused weight field only, memoised on (inventory, target), and both tap-to-autofill and generated warm-ups produce loads the active gym can actually make
...
### 21. A user can mark the equipment an exercise needs as unavailable for this workout only, and is immediately offered substitute exercises the gym can still equip.
...
### 22. A separate, explicitly-labelled action writes the unavailability through to the gym profile, behind its own confirmation — the default action never edits the profile.
...
### 23. Equipment marked unavailable is subtracted from the session's resolved inventory once, and the band, the achievability rounder and the substitute candidates all read that same subtracted view.
...
### 24. Choosing a substitute replaces the exercise for this session only, carrying the original's target sets/rep range/RIR across; the program row is untouched.
...
### 25. The Equipment overflow row is a structural exclusion (absent, not disabled) for exercises with no resolvable equipment band, e.g. bodyweight.
...
### 26. The session menu gains a fourth row, Switch Gym, between Session Note and Discard; Discard stays last and destructively styled; selecting Switch Gym closes the menu and opens the sheet with no confirmation step.
...
### 27. The Switch Gym sheet lists every non-archived gym ordered as the gym list orders them, accent-tints and labels the session's current gym, excludes archived gyms entirely, and its Manage Gyms link routes to the gym profiles list.
...
### 28. The shape of every equipment-profile JSON column and the session's unavailable-equipment column is documented in the repository, following the same structure the project's other vocabulary references use, with every field name verified against the shipped contract module
...
### 29. The whole browser durability suite, including this phase's four new specs, passes end to end on two consecutive runs, with zero skip markers in any of the four new spec files
...
### 30. The phase's validation contract names a real automated command for every requirement, and every named test file exists
...
<!-- Relocated to ROADMAP Phase 999.2 per project-wide human-verification deferral
     (user decision, 2026-08-28). Not gaps — no code defect is implied. -->

- test: "Cold Start Smoke Test"
  idea: "Kill dev server/API, clear ephemeral state, boot both from scratch, confirm migration applies and the seeded 'My Gym' profile renders with its plate band"
  deferred_at: 2026-08-28
- test: "Plate strip renders in the reserved band (06-01-D2)"
  idea: "Confirm the per-side plate stack renders inside Phase 5's reserved 40px band and the keypad digit grid does not shift"
  deferred_at: 2026-08-28
- test: "Gym Profiles click-through from the Profile tab (06-03-D1)"
  idea: "Interactive click-through of create/set-active/edit/duplicate/archive/restore, plus collapsed archived section and active-gym accent styling"
  deferred_at: 2026-08-28
- test: "Create a gym in lb and reopen it (06-04-D7)"
  idea: "Interactive lb-unit gym create/reopen round-trip on the web target, incl. stepper floor at zero"
  deferred_at: 2026-08-28
- test: "Switch Gym mid-session (06-07-D3)"
  idea: "Visual confirmation of session-menu row order, destructive Discard styling, and no-confirmation sheet behaviour"
  deferred_at: 2026-08-28
