...
| Backend connector — upload | `PowerSyncBackendConnector.uploadData` | INTEGRATE | 02-01, 02-11 | The single mutating egress, routed through this project's `apiFetch` and `/v1/sync/push`. 02-11 closes CR-01: the response body is now parsed and a rejection only completes the transaction when `isTerminalRejection` says retrying cannot help. |
| Push-side apply path — 3 of 12 `SYNCED_TABLES` | `PUSH_APPLIED_TABLES` (`workout_session`, `session_exercise`, `logged_set`) | INTEGRATE | 02-03, 02-11 | `SyncService.TABLE_MAP`'s actual coverage, now an explicit typed contract (CR-03) instead of an accident of that map's contents. |
| Push-side apply path — 9 of 12 `SYNCED_TABLES` | `PUSH_DEFERRED_TABLES` (`routine`, `routine_day`, `routine_exercise` — Phase 4; `exercise` — Phase 3; `equipment_profile`, `user_preference` — Phase 6; `personal_record` — Phase 9; `body_metric`, `progress_photo` — Phase 12) | OPT-OUT (per-table, phased) | 02-11 | Pull-only for now — no client write helper targets any of these tables today (premise-check grep pair, 02-11), so no capability is removed. Each moves to `PUSH_APPLIED_TABLES` when the phase that owns its validation rules and conflict semantics builds an apply path; `isTerminalRejection` treats a rejection against one of these as terminal so the client never retries it forever. |
