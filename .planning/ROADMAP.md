...
- [x] **Phase 13: Program Generator Volume & Selection Rework** - Fix per-exercise volume, session-budget trimming, week-level exercise variety, selection quality and frequency-aware RIR (completed 2026-09-02)
...
- [x] 12-03-PLAN.md — Progress photos: platform-split capture/downscale/store, `progress_photo`'s push apply path, the gallery, the device-absent placeholder, and photo actions (wave 2)
...
**Goal:** A generated program fits its session budget in every cycle, spreads each muscle group's weekly volume across a capped number of sets per exercise by adding a second exercise when needed, varies exercises for a muscle group across the days of the week, prefers compound and movement-pattern-covering exercises over hash tie-breaks, and scales the RIR ladder with days per week.
...
**Wave 1** *(disjoint files — 13-01 owns `packages/program-generator`, 13-02 owns the mobile copy and the docs)*

- [x] 13-01-PLAN.md — Tracer: hardest-cycle volume split, session fit that reduces sets before removing exercises, and the daysPerWeek-keyed RIR ladder
- [x] 13-02-PLAN.md — The trimmed-day sentence made true for both concessions, and the RIR ladder plus per-exercise set cap recorded in the landmark provenance doc

**Wave 2** *(blocked on 13-01 — both touch `generate.ts`)*

- [x] 13-03-PLAN.md — Selection quality: D-07's tiered sort key behind D-08's primary gate and D-06's week-level variety gate, threaded through the day loop
