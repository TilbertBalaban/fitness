...
// Same reason as body-metric.e2e-spec.ts / excluded-exercise.e2e-spec.ts:
...
function freshEmail(tag: string): string {
  const email = `e2e-dw-${tag}-${Date.now()}-${Math.floor(Math.random() * 1e6)}@example.com`;
...
function dashboardWidgetOp(id: string, data: Record<string, unknown>, op: SyncCrudOpType = 'PUT'): SyncCrudOp {
  return { op_id: randomUUID(), op, type: 'dashboard_widget', id, data };
}
...
async function dashboardWidgetRow(id: string): Promise<DashboardWidgetRow | undefined> {
  const { rows } = await pg.query(
    'SELECT id, user_id, widget_kind, position, enabled FROM dashboard_widget WHERE id = $1',
    [id],
  );
  return rows[0];
}
...
describe('dashboard_widget sync (e2e)', () => {
  it('stores a PUT with the widget kind, position and enabled flag', async () => {
    const { cookie, userId } = await signUp('happy-path');
    const id = randomUUID();

    const op = dashboardWidgetOp(id, { widget_kind: 'next_up', position: 1024, enabled: true });
    ...
    expect(row?.widget_kind).toBe('next_up');
    expect(row?.position).toBe(1024);
    expect(row?.enabled).toBe(true);
  });

  it("stores a PUT against the authenticated session's user id, never a user_id claimed in the payload (T-12-19)", async () => {
    const { cookie, userId } = await signUp('claimed-user-id');
    const id = randomUUID();

    const op = dashboardWidgetOp(id, {
      user_id: 'someone-else-entirely',
      widget_kind: 'weekly_progress',
      position: 2048,
      enabled: true,
    });
    const res = await push(cookie, [op]);
  ...
  it('rejects a PUT whose widget_kind is outside the vocabulary with invalid_field, and writes no row (T-12-20)', async () => {
    const { cookie } = await signUp('bad-kind');
    const id = randomUUID();

    const op = dashboardWidgetOp(id, { widget_kind: 'not_a_real_widget', position: 1024, enabled: true });
    const res = await push(cookie, [op]);
  ...
  it('rejects a PUT whose position is not a valid non-negative integer with invalid_field, and writes no row (T-12-20)', async () => {
    const { cookie } = await signUp('bad-position');
    const id = randomUUID();

    const op = dashboardWidgetOp(id, { widget_kind: 'next_up', position: -1, enabled: true });
    const res = await push(cookie, [op]);
    expect((res.body as SyncPushResponse).rejected).toEqual([{ op_id: op.op_id, reason: 'invalid_field' }]);
    expect(await dashboardWidgetRow(id)).toBeUndefined();
  });
