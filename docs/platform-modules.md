...
| Photo capture (`expo-image-picker` 57.0.14, `lib/photos/capture.ts` / `capture.web.ts`) | Yes | **Split by design, not by discovery.** Native (`capture.ts`) requests media-library permission and launches `ImagePicker.launchImageLibraryAsync`; the web sibling takes no new dependency at all (D-16) — a bare `<input type="file" accept="image/*">`, clicked and resolved on `change`. `expo export --platform web` resolving `capture.web.ts` with no `expo-image-picker` import in the emitted bundle is the observed confirmation (Task 2 acceptance criteria grep). Native launch behaviour is unbuildable and unverifiable on this machine (no Xcode, no Android SDK) — filed as a `.planning/WINDOWS.md` unrun-verify entry against Phase 999.1. | **`.web.ts` sibling** (already built) — no `Platform.OS` branch in either file. | 12-03 (Progress Photos) |
| Photo re-encode/downscale (`expo-image-manipulator` 57.0.14, `lib/photos/downscale.ts` / `downscale.web.ts`) | Yes | **Split by design.** Native uses the current contextual `ImageManipulator.manipulate(uri).resize(...).renderAsync().saveAsync(...)` API; the web sibling draws onto an offscreen `<canvas>` and re-encodes through `canvas.toBlob`, no `expo-image-manipulator` import at all (confirmed by the same web-bundle-absence check as the row above). Both siblings share the same pure `resolveDownscaledDimensions` helper so the D-17 bound (`PHOTO_MAX_LONG_EDGE`) resolves to identical target dimensions on either platform. Native re-encode is unbuildable/unverifiable here — filed as a `.planning/WINDOWS.md` unrun-verify entry against Phase 999.1. | **`.web.ts` sibling** (already built) — no `Platform.OS` branch in either file. | 12-03 (Progress Photos) |
| App-private photo storage (`expo-file-system` `File`/`Paths`, `lib/photos/photo-store.ts` / `photo-store.web.ts`) | Yes (already installed for `export-training-data.ts`) | **No web equivalent — this is RESEARCH Pitfall 1's own example.** `File`/`Directory`/`Paths` compile cleanly for web and throw at runtime; there is no app-private document-directory API in a browser. The web sibling stores JPEG `Blob`s in a dedicated `indexedDB` database keyed by `storage_key` instead — no new npm dependency (RESEARCH Standard Stack). `hasPhotoBytes`/`getPhotoUri`/`putPhotoBytes`/`deletePhotoBytes` are the same four exported names on both siblings, so `lib/db/progress-photos.ts` and `app/progress-photos.tsx` never branch on platform. Proven against a real browser's real IndexedDB in `e2e/progress-photo.spec.ts` (the `durability` Playwright project); native file-store behaviour is unbuildable/unverifiable here — filed as a `.planning/WINDOWS.md` unrun-verify entry against Phase 999.1. | **`.web.ts` sibling** (already built) — no `Platform.OS` branch in either file. | 12-03 (Progress Photos) |
| Before/after composite render + handoff (`react-native-view-shot` 5.1.0, `expo-sharing`, `lib/photos/composite.ts` / `composite.web.ts`) | Yes (`react-native-view-shot` new this phase; `expo-sharing` already installed) | **Split by design, not by discovery.** Native (`composite.ts`) renders a hidden composed `CompositeCaptureView` (two photos + date captions, geometry shared with the web sibling via `composite-layout.ts`'s `resolveCompositeCanvas`), snapshots it with `react-native-view-shot`'s `captureRef`, and hands the resulting file uri to `expo-sharing`'s `shareAsync`. The web sibling (`composite.web.ts`) draws both source images onto an offscreen `<canvas>` and delivers a `Blob` through the same `<a download>` idiom `export-training-data.web.ts` established — `expo-sharing` cannot share a local file by uri on web at all (RESEARCH Pitfall 2), so the web sibling never imports it, and `composite.ts`'s `CompositeCaptureView` is a no-op on web so photo-composite.tsx's single call site never branches on platform. Proven end-to-end against a real browser's `<canvas>` and a real Playwright download event in `e2e/photo-composite.spec.ts`; native `captureRef`/`expo-sharing` behaviour is unbuildable/unverifiable here — filed as a `.planning/WINDOWS.md` unrun-verify entry against Phase 999.1 (RESEARCH Assumption A1: `react-native-view-shot`'s New-Architecture support for this project's exact SDK 57 / RN 0.86.2 pairing is asserted, not confirmed on-device). | **`.ts`/`.web.ts` sibling pair** (already built) — no `Platform.OS` branch in either file. | 12-06 (Before & After Composite) |
