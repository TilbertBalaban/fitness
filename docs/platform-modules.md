...
| Notifications (`expo-notifications`) | No | **Unverified.** Expected to require the browser Push/Notification APIs, user permission, a service worker, and an HTTPS origin, and to be unable to fire while the tab is closed the way a native local notification fires with the app backgrounded. Not observed — the module is not installed, so nothing was measured. | **Undecided.** Likely a shared `notify()` module with a `.web.tsx`/`.web.ts` sibling rather than a call-site branch. Decide when it is first needed, and add a row here with an observed behaviour at that time. | Phase 5 — the rest timer must alert with the app backgrounded and the phone locked (ROADMAP Phase 5 success criterion 3) |
